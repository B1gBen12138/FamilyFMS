            ...
            <table width="98%" border="0" cellspacing="0" cellpadding="0">
                <tr align="center" class="bg03">
                    ...
                    <c:if test="${account.isAdmin == true}">
                        <td width="15%" class="text007">用户</td>
                    </c:if>
                    <td width="15%" class="text007"> 证券账户</td>
                    <%--                    <td width="35%" class="text007"》</td>>--%>
                    <td width="15%" class="text007"> 证券</td>
<%--                    <td width="30%" class="text007">操作</td>--%>
                </tr>
                <c:if test="${account.isAdmin == true}">

                </c:if>
                <c:forEach var="dataMap" items="${bondList}" varStatus="vs">
                    <c:set var="accountKey" value="${dataMap.key}"/>
                    <c:set var="bondAccountValue" value="${dataMap.value}"/>
                    <c:forEach var="bondAccountMap" items="${bondAccountValue}" varStatus="bondvs">
                        <c:set var="bondAccountKey" value="${bondAccountMap.key}"/>
                        <c:set var="bondListValue" value="${bondAccountMap.value}"/>
                        <c:forEach var="bondListItem" items="${bondListValue}" varStatus="listvs">
